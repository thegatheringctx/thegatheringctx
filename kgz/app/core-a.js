...
// ── ARMOR HELPER ─────────────────────────────────
function getArmor(kid){
  var pts=(kid.lifetime_points!=null?kid.lifetime_points:(kid.points||0)),u=[];
  if(pts>=25)u.push("belt");if(pts>=100)u.push("breastplate");
  if(pts>=250)u.push("boots");if(pts>=500)u.push("shield");
  if(pts>=800)u.push("helmet");if(pts>=1200)u.push("sword");
  if(pts>=1800)u.push("full");
  return u;
}
...
function renderVerse(){var el=document.getElementById("tab-verse");if(!el)return;el.innerHTML="<p class='muted' style='text-align:center;padding:2rem'>Loading...</p>";var grp=APP.kid?APP.kid.age_group:"812";sb("memory_verses?active=eq.true&age_group=eq."+grp+"&order=sort_order.asc").then(function(rows){el.innerHTML="";var hdr=document.createElement("div");hdr.style.cssText="margin-bottom:1rem";hdr.innerHTML="<div style='font-family:Bangers,cursive;font-size:1.7rem;letter-spacing:.04em'>📜 Hide It In Your Heart</div><div class='muted'>Memorize this week's verse. Say it from memory to earn points.</div>";el.appendChild(hdr);if(!rows||!rows.length){var e=document.createElement("div");e.className="muted";e.style.cssText="text-align:center;padding:2rem";e.textContent="New verse coming soon, warrior!";el.appendChild(e);return;}versePractice(el,rows[0],(APP.kid&&APP.kid.completed_verses)?APP.kid.completed_verses.indexOf(rows[0].id)>=0:false);}).catch(function(){el.innerHTML="<p class='muted' style='text-align:center;padding:2rem'>Could not load.</p>";});}
