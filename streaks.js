...
(function(){
 ...
 var STICKERS=[
  {id:'first',    ic:'star',   n:'First Steps',    hint:'Join the warriors',        on:function(){return true;}},
  {id:'verse1',   ic:'book',   n:'Verse Keeper',   hint:'Learn your first verse',    on:function(k){return vlen(k)>=1;}},
  {id:'devo1',    ic:'heart',  n:'Quiet Time',     hint:'Finish your first devo',    on:function(k){return dlen(k)>=1;}},
  {id:'streak3',  ic:'flame',  n:'On Fire',        hint:'Keep a 3-day streak',       on:function(k){return best(k)>=3;}},
  {id:'armor3',   ic:'shield', n:'Armored Up',     hint:'Earn 3 armor pieces',       on:function(k){return pieces(k)>=3;}},
  {id:'verse5',   ic:'sword',  n:'Word Warrior',   hint:'Learn 5 verses',            on:function(k){return vlen(k)>=5;}},
  {id:'devo5',    ic:'book',   n:'Faithful Reader',hint:'Finish 5 devos',            on:function(k){return dlen(k)>=5;}},
  {id:'mission1', ic:'target', n:'Truth Scout',    hint:'Finish a training mission', on:function(k){return mlen(k)>=1;}},
  {id:'watch1',   ic:'play',   n:'All Ears',       hint:'Watch a warrior video',     on:function(k){return wlen(k)>=1;}},
  {id:'streak7',  ic:'bolt',   n:'Unstoppable',    hint:'Keep a 7-day streak',       on:function(k){return best(k)>=7;}},
  {id:'streak14', ic:'trophy', n:'Legendary',      hint:'Keep a 14-day streak',      on:function(k){return best(k)>=14;}},
  {id:'armorfull',ic:'full',   n:'Fully Armored',  hint:'Wear the full armor of God',on:function(k){return hasFull(k);}}
 ];
 ...
 function build(k){
  ...
  STICKERS.forEach(function(s){
   ...
   badges+="<div class='stk-badge "+(got?'got':'locked')+(isFresh?' pop':'')+"'>"+
     "<span class='em'>"+(window.wzIcon?wzIcon(s.ic,30):'')+"</span>"+
  ...
  if(fresh.length){
   setTimeout(function(){
    ...
    if(s0&&typeof toast==='function')toast('New sticker: '+s0.n+(fresh.length>1?' (+'+(fresh.length-1)+' more)':'')+'!',4500);
   },600);
  }
  return card;
 }

 // Swap the Hall of Victory's emoji headers for the custom icon set.
 function swapHallHeaders(el){
  if(!window.wzIcon)return;
  var map={'The Gathering Warriors':'trophy','Your Journey':'sword','Wall of Warriors':'shield'};
  [].forEach.call(el.querySelectorAll('.bl-t'),function(t){
   if(t.__wzIc)return;
   var txt=t.textContent||'';
   for(var name in map){ if(txt.indexOf(name)>=0){ t.innerHTML=wzIcon(map[name],18)+' '+name; t.__wzIc=1; break; } }
  });
 }
